...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQ03PD2138_V1_SENSOR_ID,
	...
	.pre = {
		...
		.mipi_pixel_rate = 330200000,  ////mipi 1651.0  
	...
	.normal_video = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 592800000,  ////mipi 1482.0
	...
	.hs_video = { /*hs_video 120fps*/
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 592800000, //1482
	...
	.slim_video = { /* hs_video 240fps*/
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 275600000, ////mipi 1378.0
  ...
  .custom3 = { /*sw remosiac */
		...
		.grabwindow_height = 5984,
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 772400000,  ////mipi 1391.0
	...
	.custom4 = { /* 2032*1536@15fps */
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 350130000,/*511200000*/  ////mipi 876.0
	...
	.custom5 = { /* 3840*2160@60fps */
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 821600000,/*511200000*/  ////mipi 2054.0
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {  
	...
	{8160, 6144,    0,    0, 8000, 5984, 8000, 5984, 0, 0, 8000, 5984, 0, 0, 8000, 5984}, /* sw remosiac*/
...
{/* Preview mode setting 4080*3060*/
	...
	/* custom2 mode setting 4080*3060 30fps*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x1200, 0x0D70, 0x00, 0x00, 0x00, 0x00,
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4080X3072
{
	...
	.i4BlockNumY = 382,
	.iMirrorFlip = 3,
	.i4Crop = { {0,6}, {0,6}, {0, 388}, {0, 0}, {0, 0}, {408,312}, {120, 456}, {0, 0}, {0, 0}, {120, 456}},
};
#endif
/*hope add otp check start*/
static int vivo_otp_read_when_power_on;
extern int MAIN_38E1_otp_read_v1(void);
extern otp_error_code_t S5KJN1SQ03_V1_OTP_ERROR_CODE;
MUINT32  sn_inf_main_s5kjn1sq03v1[13];  /*0 flag   1-12 data*/
MUINT32  crosstalk_data_PD2147v1[PDAF_DATA_SIZE - 2];  /*0 flag   1-836 data*/
MUINT32  material_inf_main_s5kjn1sq03v1[4];
MUINT32  module_v1_Id[1];
...
static int s5kjn1sq03PD2147_v1_read_framelength;
static int s5kjn1sq03PD2147_v1_read_shutter;
static int s5kjn1sq03PD2147_v1_read_gain;
#endif
...
#ifdef READ_FRAMELENGTH_SHUTTER_GAIN
	s5kjn1sq03PD2147_v1_read_framelength = read_cmos_sensor_16_16(0x0340);
	s5kjn1sq03PD2147_v1_read_shutter = read_cmos_sensor_16_16(0x0202);
	PK_DBG("s5kjn1sq03PD2147_v1_read_shutter = %d, s5kjn1sq03PD2147_v1_read_framelength = %d \n", s5kjn1sq03PD2147_v1_read_shutter,s5kjn1sq03PD2147_v1_read_framelength);
#endif
}	/*	write_shutter  */
...
static kal_uint16 addr_data_pair_preview[] = {
    ...
    0x0312, 0x0000,
    0x080E, 0x0000,
    0x0340, 0x0C70, ////FLL 0FF8
    0x0342, 0x1700, ////LLP 11E8
    0x0702, 0x0000,
    0x0202, 0x0100,
    0x0200, 0x0100,
    0x0D00, 0x0101,
...
static kal_uint16 addr_data_pair_hs_video[] = {
    ...
    0x030A, 0x0001,
    0x030C, 0x0000,
    0x030E, 0x0004,  ////mipi div 0003
    0x0310, 0x0072,  ////mipi multi 005D
    0x0312, 0x0000,
    0x080E, 0x0000,
    0x0340, 0x0782, ///FLL 0930
    0x0342, 0x0A20, ///LLP 0844
    0x0702, 0x0000,
    0x0202, 0x0100,
    0x0200, 0x0100,
    0x0D00, 0x0101,
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000)+1 ;
			...
			if ( *sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = MAIN_38E1_otp_read_v1();
				if(module_v1_Id[0] == 0x9)
				{
					pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_F8D1_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KJN1SQ03_V1_OTP_ERROR_CODE);
					/*vivo lxd add end*/
					return ERROR_NONE;	
				}else{
					*sensor_id = 0xFFFFFFFF;
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJN1SQ03_V1_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sq03v1, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_s5kjn1sq03v1, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjn1sq03v1[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sq03v1[i]);
						}
				#endif
			}
				break;
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			for(i = 0; i < PDAF_DATA_SIZE - 2 ; i++) {
				data[2 + i] = crosstalk_data_PD2147v1[i];
			}
		}
 	}
  	  break;
...
UINT32 S5KJN1SQ03PD2147_V1_MIPI_RAW_SensorInit(
