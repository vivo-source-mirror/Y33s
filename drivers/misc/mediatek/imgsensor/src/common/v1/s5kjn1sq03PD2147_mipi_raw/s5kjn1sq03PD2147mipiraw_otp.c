...
extern MUINT32  module_Id[1];
extern MUINT32 crosstalk_data_PD2147[PDAF_DATA_SIZE - 2];  /*0 flag   1-606 data*/
...
int MAIN_38E1_otp_read(void)
{
    ...
    module_Id[0] = vivo_otp_data_s5kjn1sq03[1];
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		for (i = 0; i < PDAF_DATA_SIZE - 2; i++) {
			...
			crosstalk_data_PD2147[i]=vivo_otp_data_s5kjn1sq03[XTC_cal_addr + i + index];
			}
