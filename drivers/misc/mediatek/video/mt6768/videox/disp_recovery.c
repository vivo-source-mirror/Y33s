...
static int te_irq;
unsigned int lcm_need_read_esd_once = 1;
...
#ifdef CONFIG_LCM_ESD_CHECK_RELOAD_FW
extern int vts_node_reset_fw_download(enum display_ndx type);
#endif
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		...
		if (ret == 1) {
			...
			recovery_done = 0;
#ifdef CONFIG_LCM_ESD_CHECK_RELOAD_FW
			if (params->dsi.esd_check_reload_firmware == 1)
				vts_node_reset_fw_download(PRIMARY_DISPLAY);
#endif
		}
		exit_esd_check:
		esd_checking = 0;
