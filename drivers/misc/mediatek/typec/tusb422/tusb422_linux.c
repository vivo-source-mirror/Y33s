...
void vivo_set_tusb422_power_role(int new_role)
{
	if (tusb422_probe_success) {
		if (new_role == 2) {
			new_role_test = 2;
			last_role = new_role_test;
			charger_changed_role = false;
		}
		else if (new_role == 1) {
			new_role_test = 1;
			last_role = new_role_test;
			...
			pr_err("%s: charger plug_in, last_role[%d], charger_flag[%d] \n", __func__, last_role, charger_changed_role);
			if ((last_role == ROLE_DRP) && !charger_changed_role) {
				new_role_test = ROLE_SNK;
				charger_changed_role = true;
				pr_err("%s: usb plug_in! swap role to snk \n", __func__);
			} else if (last_role == ROLE_SNK || last_role == ROLE_SRC) {
				pr_err("%s: last_role is snk, nothing to do \n", __func__);
				return;
			}
		} else if (new_role == 4) {
			pr_err("%s: charger plug_out, last_role[%d], charger_flag[%d] \n", __func__, last_role, charger_changed_role);
			if ((last_role == ROLE_DRP) && charger_changed_role) {
				new_role_test = ROLE_DRP;
				charger_changed_role = false;
				pr_err("%s: usb plug_out! swap role to drp \n", __func__);
			}
		}
...
void vivo_inquire_tusb422_cc_status(uint8_t *cc1, uint8_t *cc2, bool from_ic)
{
	if (tusb422_probe_success) {
		...
		pr_info("%s: TUSB422 not probe success!!! \n", __func__);
	}
}
