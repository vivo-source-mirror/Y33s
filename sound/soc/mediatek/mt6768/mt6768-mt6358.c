...
static int module_ver_gpio = 0;
static int module_ver_gpio_val = 0;

static int mic_ver_gpio = 0;
static int mic_ver_gpio_val = 0;
...
#endif
//baoqiang qin add for codec begin
	{
	 .name = "PACODECPLAYBACK",
	 .stream_name = "PAcodec_Playback",
	 .cpu_dai_name = "snd-soc-dummy-dai",
...
static int class_attr_create(struct device_node *np)
{
	...
	if (ret < 0) {
		...
		if ((module_ver_gpio_val == 1) && (mic_ver_gpio_val == 0)) {
			product_name = "PD2138F_EX_DM";
			pr_info("%s: product_name: %s\n",__func__,product_name);
		} else if ((module_ver_gpio_val == 1) && (mic_ver_gpio_val == 1)) {
			product_name = "PD2138F_EX_SM";
			pr_info("%s: product_name: %s\n",__func__,product_name);
		} else if ((module_ver_gpio_val == 0) && (mic_ver_gpio_val == 0)) {
			product_name = "PD2147F_EX_DM";
			pr_info("%s: product_name: %s\n",__func__,product_name);
		} else if ((module_ver_gpio_val == 0) && (mic_ver_gpio_val == 1)) {
			product_name = "PD2147F_EX_SM";
			pr_info("%s: product_name: %s\n",__func__,product_name);
		} else {
			pr_info("%s: default product_name: %s\n",__func__,product_name);
		}
	}
...
static int mt6768_mt6358_dev_probe(struct platform_device *pdev)
{
	...
	module_ver_gpio = of_get_named_gpio(pdev->dev.of_node, "module-version-gpio", 0);
	if (gpio_is_valid(module_ver_gpio)) {
		pr_info("%s: get modul ver gpio name success\n",__func__);
		ret = devm_gpio_request_one(&pdev->dev, module_ver_gpio,
			GPIOF_IN, "module-version-gpio");
		if (ret) {
			pr_err("%s module version GPIO requeset failed %d.\n", __func__, ret);
			devm_gpio_free(&pdev->dev, module_ver_gpio);
		} else {
			module_ver_gpio_val = gpio_get_value(module_ver_gpio);
			pr_info("%s: module version GPIO val %d.\n", __func__, module_ver_gpio_val);
		}
	...
	mic_ver_gpio = of_get_named_gpio(pdev->dev.of_node, "mic-version-gpio", 0);
	if (gpio_is_valid(mic_ver_gpio)) {
		pr_info("%s: get mic ver gpio name success\n",__func__);
		ret = devm_gpio_request_one(&pdev->dev, mic_ver_gpio,
			GPIOF_IN, "mic-version-gpio");
		if (ret) {
			pr_err("%s mic version GPIO requeset failed %d.\n", __func__, ret);
			devm_gpio_free(&pdev->dev, mic_ver_gpio);
		} else {
			mic_ver_gpio_val = gpio_get_value(mic_ver_gpio);
			pr_info("%s: mic version GPIO val %d.\n", __func__, mic_ver_gpio_val);
		}			
	} else {
		pr_err("%s: get modul mic gpio name failed\n",__func__);
	}
	...
	//vivo audio add for project conf
